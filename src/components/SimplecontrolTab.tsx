...
import { GripperState, DataPoint, mAtoNm, PH11_SPEC } from '../App';
  ...
  return (
    <Box sx={{ bgcolor: '#fff', border: '1px solid #E0EAF4', borderRadius: 1, p: '6px 10px', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
      ...
      <Typography sx={{ fontSize: '0.62rem', color: '#B0BEC5' }}>
        {val <= PH11_SPEC.rated ? '● Rated range' : val <= PH11_SPEC.peak ? '▲ Over rated' : '⚠ Exceeded peak'}
      </Typography>
    </Box>
  );
};
  ...
  <Box>
    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
      ...
      <Typography sx={{ fontSize: '0.68rem', fontWeight: 600, color: disabled ? '#C5D0D8' : '#37474F' }}>
        {value}
        <span style={{ fontSize: '0.58rem', color: '#B0BEC5', marginLeft: 2 }}>/100</span>
      </Typography>
    </Box>
    <Slider
      size="small" min={0} max={100} value={value} disabled={disabled}
...
const paperSx = {
  sx: {
    ...
    border: '1px solid #EEF2F7', borderRadius: 2,
    bgcolor: '#fff',
  },
};

// EYOU PH11B-51-C spec
const Y_MAX   = 20;                      // chart ceiling (Nm) — above 17 Nm peak
const Y_RATED = PH11_SPEC.rated;         // 3.5 Nm rated
const Y_PEAK  = PH11_SPEC.peak;          // 17  Nm instantaneous max

const SimpleControlTab: React.FC<Props> = ({ state, updateState, chartData }) => {
  // Convert mA → Nm using App-exported helper
          ...
          <Box sx={{ flex: 1, minHeight: 0 }}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData} margin={{ top: 2, right: 6, left: -14, bottom: 14 }}>
                ...
                <YAxis domain={[0, 100]} tickCount={6} {...axisStyle} />
        ...
        <Paper elevation={0} {...paperSx} sx={{ ...paperSx.sx, flex: 1, p: '12px 14px 6px' }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 0.6 }}>
            <Box>
              ...
              <Typography sx={{ fontSize: '0.58rem', color: '#B0BEC5', ml: 0.8, display: 'inline' }}>
                PH11 · rated {Y_RATED} Nm · peak {Y_PEAK} Nm
              </Typography>
            </Box>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={forceData} margin={{ top: 2, right: 36, left: 2, bottom: 14 }}>
                ...
                <XAxis dataKey="time" tickFormatter={v => `${Number(v).toFixed(0)}s`} {...axisStyle}
                  label={{ value: 'Time (s)', position: 'insideBottom', offset: -4, style: { fontSize: 9, fill: '#C5D0D8' } }} />
                <YAxis
                  domain={[0, Y_MAX]} tickCount={5} {...axisStyle}
                  tickFormatter={v => `${v}`}
                  unit=" Nm"
                />
                <Tooltip content={<ForceTooltip />} />

                {/* Rated torque line — 3.5 Nm */}
                <ReferenceLine
                  y={Y_RATED} stroke="#90CAF9" strokeDasharray="4 2"
                  label={{ value: `Rated ${Y_RATED} Nm`, position: 'right', style: { fontSize: 8, fill: '#90CAF9' } }}
                />
                {/* Peak torque line — 17 Nm */}
                <ReferenceLine
                  y={Y_PEAK} stroke="#FFCDD2" strokeDasharray="4 2"
                  label={{ value: `Peak ${Y_PEAK} Nm`, position: 'right', style: { fontSize: 8, fill: '#EF9A9A' } }}
                />

                <Line
                  type="monotoneX" dataKey="forceNm"
                  stroke="#E64A19" strokeWidth={2}
                  dot={false} isAnimationActive={false} name="Force"
                />
              </LineChart>
            </ResponsiveContainer>
          </Box>
        ...
        <Paper elevation={0} {...paperSx} sx={{ ...paperSx.sx, flex: 1, p: '12px 16px', justifyContent: 'space-between' }}>
          ...
          <Box sx={{ display: 'flex', gap: 1 }}>
            {[{ label: 'OPEN', val: 0 }, { label: 'CLOSE', val: 100 }].map(({ label, val }) => {
              ...
              return (
                <Button
                  key={label} fullWidth size="small"
          ...
          {/* Sliders — max 100, initial: pos=0, speed=50, force=30 */}
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.4 }}>
            <ParamSlider
              label="Position" value={state.positionRequest}
              disabled={!state.goToPosition || state.eStop}
