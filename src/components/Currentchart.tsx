  ...
  return (
    <Box sx={{ bgcolor: '#fff', border: '1px solid #E0EAF4', borderRadius: '4px', p: 1, boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
      ...
      <Typography sx={{ fontSize: '0.72rem', color: '#E64A19', fontWeight: 500 }}>
        Current: {Math.round(payload[0]?.value ?? 0)} mA
      </Typography>
    </Box>
  );
};
...
const CurrentChart: React.FC<{ chartData: DataPoint[] }> = ({ chartData }) => {
  const maxCurrent = Math.max(...chartData.map(d => d.current), 500);
  ...
  return (
    <Paper sx={{ p: 2, bgcolor: '#fff', display: 'flex', flexDirection: 'column', height: '100%' }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1, pb: 0.6, borderBottom: '1px solid #E0EAF4' }}>
        <Typography sx={{ fontSize: '0.72rem', fontWeight: 600, color: '#1976D2' }}>Force Actual (mA)</Typography>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
          <Box sx={{ width: 16, height: 2, bgcolor: '#E64A19', borderRadius: 1 }} />
          <Typography sx={{ fontSize: '0.65rem', color: '#90A4AE' }}>Current</Typography>
        </Box>
      </Box>
      <Box sx={{ flex: 1, minHeight: 0 }}>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} margin={{ top: 4, right: 8, left: -4, bottom: 0 }}>
            ...
            <ReferenceLine y={2000} stroke="#FFCDD2" strokeDasharray="4 2" label={{ value: 'Max', position: 'right', style: { fontSize: 8, fill: '#EF9A9A' } }} />
            <Line type="monotoneX" dataKey="current" stroke="#E64A19" strokeWidth={2} dot={false} isAnimationActive={false} name="Current" />
          </LineChart>
        </ResponsiveContainer>
      </Box>
    </Paper>
